...
export const useCreateMyRestaurant=()=>{
    ...
    const {mutateAsync:createRestaurant,isLoading,isSuccess,error}=useMutation(createMyRestaurantRequest);
...
export const useUpdateMyRestaurantOrder=()=>{
    const {getAccessTokenSilently}=useAuth0();
    const useUpdateMyRestaurantOrder=async (updateStatusOrderRequest:UpdateOrderStatusRequest)=>{
        const accessToken=await getAccessTokenSilently();
    ...
    const {mutateAsync:updateRestaurantStatus,isLoading,isError,isSuccess,reset}=useMutation(useUpdateMyRestaurantOrder);
    if(isSuccess){
    ...
    if(isError){
     ...
     reset();//ensuring error toast wont appear everytime component renders
    }
