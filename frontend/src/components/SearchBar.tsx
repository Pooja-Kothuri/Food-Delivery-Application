...
const SearchBar = ({onSubmit,placeHolder,onReset,searchQuery}:Props) => {
   ...
   const handleReset=()=>{
    ...
    if(onReset){
        //if onReset has a value
        onReset();
    }
   }
    return ( 
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`flex items-center  gap-3 justify-between flex-row border-2  rounded-full p-[0.3rem]  ${form.formState.errors.searchQuery && "border-red-500"}`}>
